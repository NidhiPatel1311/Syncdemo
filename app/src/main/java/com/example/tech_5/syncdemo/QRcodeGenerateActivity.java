        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(etqr.getText().toString().trim().length() == 0){
                    Toast.makeText(QRcodeGenerateActivity.this, "Enter String!", Toast.LENGTH_SHORT).show();
                }else {
                    try {
                        bitmap = TextToImageEncode(etqr.getText().toString());
                        iv.setImageBitmap(bitmap);
                        String path = saveImage(bitmap);  //give read write permission
                        Toast.makeText(QRcodeGenerateActivity.this, "QRCode saved to -> "+path, Toast.LENGTH_SHORT).show();
